import { Check, EllipsisVertical, Plus, TextAlignJustify } from 'lucide-react'
import { useEffect, useState } from 'react'

const Today = ({ lists, setOpenMobileMenu, setTodayTask, addTodayTask, setAddTodayTask, addTask, randomColor, setSelectedTaskId }) => {
  ...
  return (
    <div className='w-full lg:px-3 lg:py-3'>
      <div className='flex items-center gap-5 pb-4 border-b-2 border-(--Border)/15'>
        ...
        <h3 className='text-(--Text-Primary) text-xl font-bold'>Today</h3>
        <div className='bg-(--Surface)/60 rounded px-3 py-0.5 text-xl font-semibold text-(--Text-Primary)'>
          {addTodayTask.length}
        </div>
      </div>
      <div onClick={() => setTodayTask(true)} className='group mt-2 flex items-center gap-3 cursor-pointer border-b-2 border-(--Border)/15 pl-3 pt-2 pb-3'>
            ...
            <div key={task.id} onClick={() => {
              ...
              <div className='flex items-center justify-between'>
                <div onClick={(e) => {
                  ...
                  <p className={`${isComplete ? "line-through" : ""} text-(--Text-Primary)/95 transition-all duration-400 font-medium text-md`}>{task.task}</p>
                </div>
                ...
                {dropDown === task.id && <div className='absolute right-10 mt-15 bg-(--Background) p-1 rounded-md shadow'>
                  <div onClick={(e) => {
                    e.stopPropagation()
                    setAddTodayTask(prev => prev.filter(t => t.id !== task.id))
